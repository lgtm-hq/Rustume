...
| Section-heading chrome | fix-in-typst | Still open, but narrower than it reads on older per-template pages. The sheet is NOT one `.doc-sheet__sec-title` treatment: `bundledTemplateLayout` already selects per-template chrome (pikachu `headingStyle: "band"` + `sidebarHeadingStyle: "plain"`; rhyhorn / bronzor / onyx / kakuna / azurill / chikorita / ditto / gengar underline + `headingCase: "upper"`; glalie underline + `as-written`; nosepass `rule`), and `docSheet.css` paints them as `.doc-sheet--heading-*` / `.doc-sheet--side-heading-*` modifiers. What is still open is only where Typst's extras — rule weight, dual column scales, band radius — exceed those modifiers. Do not retag a heading row `fix-in-typst` on the claim that the sheet lacks the chrome. |
...
| Column padding on full-bleed templates (pikachu, ditto, gengar, glalie) | fixed (#919) | Closed: insets follow the sheet's three-value paddings, `.doc-sheet__side` `1.6rem 0.95rem 2rem` and `.doc-sheet__main` `1.6rem 1.45rem 2rem`, at 1rem = 12pt. Typst insets are symmetric in y, so the 1.6rem top is used for both edges; the sheet's larger 2rem bottom is slack under a scrolling column. Single-column `page.margin` stays at 48pt, and the cover-letter page keeps its own inset — the sheet has no opinion on either. |
...
| Level glyphs | fixed (#919) | Closed: `template-default` is the sheet's five dots on all 12 templates (`sheet-level-dots()`), flat accent over a flat `#d6d3d1` track, no outline. Sizes use the same px→pt mapping as the column padding (1rem = 16px = 12pt), so `.doc-sheet__lang-dots i` at 6px with a 3px gap is 4.5pt with a 2.25pt gap. Explicit `metadata.levelDisplay` overrides keep `render-level`'s outlined indicators. |
| Skill / interest keywords | fixed (#919) | Closed, per template, against the sheet's own `keywordStyle`. Where `bundledTemplateLayout().keywordStyle` is `chips` (azurill, chikorita, ditto, gengar, kakuna, leafish, onyx), `render-item-tag-chips()` paints `.doc-sheet__tag-chip` — 999pt radius, `color-mix(accent 10%, bg)` fill, `color-mix(accent 28%, #e7e5e4)` border, and the label in BODY ink at weight 600, since the CSS rule sets no `color`. Where it is `plain` (rhyhorn, bronzor, glalie, pikachu, nosepass), `.doc-sheet--keywords-plain` renders comma-separated muted text and Typst does the same — do not convert those to chips. nosepass keeps its boxed skill pill; the pill is already the chip there. Project / custom keywords stay per-template. |
