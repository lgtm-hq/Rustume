...
| 4.1.3 Status Messages — result count | **fixed** ([#673](https://github.com/lgtm-hq/Rustume/issues/673)) | *As measured in this pass:* query `resume`: `messageText: "31 results for resume"`, `resultCount: 5`, **live regions inside `#search-menu`: 0**. The count arrives silently. *Since fixed:* an `sr-only` `role="status" aria-live="polite" aria-atomic="true"` region inside `#search-menu` mirrors Pagefind's `.pagefind-ui__message`, debounced 400 ms. Mirrored rather than decorated because Pagefind owns and replaces that subtree. The results drawer is deliberately **not** live — it repaints per keystroke. Two things are scoped to the open dialog rather than to the page: the region is emptied on close, so the write-deduplication guard (an unchanged count is not re-announced) cannot swallow the announcement when a query is repeated after reopening; and the observer callback returns early unless the dialog is open, so Pagefind's continued repainting of its own container cannot re-arm the debounce and read a count out over a dismissed dialog. Regression tests: `apps/site/e2e/chrome.spec.ts`, "search result count is announced politely, including no results" (covers the zero case), "typing a query collapses to fewer announcements than keystrokes", "a dismissed dialog announces nothing when Pagefind keeps repainting", and "repeating a query after reopening still announces the count". |
