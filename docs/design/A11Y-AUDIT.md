...
| 1.3.1 Heading structure — footer | pass ([#676](https://github.com/lgtm-hq/Rustume/issues/676)) | `.footer-col` headings in `Footer.astro` ("Docs", "Community") are now `<h2>`s — peers of the page's own top-level sections, not subsections of whatever came last — with the old `h4` type scale kept via CSS so the visual size is unchanged. No level skip on `/`, `/docs/`, `/faq/`, `/cloud/` or docs articles. `accessibility.spec.ts` now enables axe's `best-practice`-tagged `heading-order` rule (with the `assertRuleEvaluated` guard) and `heading-outline.spec.ts` walks the DOM outline directly so a WCAG-tag change alone cannot silently drop the coverage again. |
...
| Criterion | Status | Evidence |
| --- | --- | --- |
| 2.4.3 Focus Order — containment | **fixed** ([#672](https://github.com/lgtm-hq/Rustume/issues/672)) | *As measured in this pass:* the `#search-menu` panel declares `role="dialog"` and the trigger declares `aria-haspopup="dialog"`, but nothing contains focus. With the dialog open and `opacity: 1`: `aria-modal: null`, `main[inert]: false`, `main[aria-hidden]: null`. Six Tab presses, dialog still open the whole time: "Select theme" → "GitHub" → "CLI" → "Rustume Cloud" → "Pricing" → "API", **all outside the dialog**. One Tab leaves. *Since fixed:* `openSearch()` sets `aria-modal="true"` and inerts the page around the panel, and a Tab handler wraps within it. Note the panel lives **inside** `body > header`, so the template gallery's flat `body > header, body > main, body > footer` selector could not be reused verbatim — `inert` inherits, and it would have inerted the dialog itself; the fix walks the panel's ancestor chain and inerts each level's siblings, which reaches `body > main` and `body > footer` and additionally the header's own controls. Regression test: `apps/site/e2e/chrome.spec.ts`, "search dialog keeps Tab inside it and inerts the page behind". |
| 2.4.3 Focus Order — on open and on Escape | pass | Focus lands on the Pagefind input on open (`focusInside: true`), via the documented cross-frame retry in `focusSearchInput()`. This is #620 holding. |
| 4.1.3 Status Messages — result count | **fixed** ([#673](https://github.com/lgtm-hq/Rustume/issues/673)) | *As measured in this pass:* query `resume`: `messageText: "31 results for resume"`, `resultCount: 5`, **live regions inside `#search-menu`: 0**. The count arrives silently. *Since fixed:* an `sr-only` `role="status" aria-live="polite" aria-atomic="true"` region inside `#search-menu` mirrors Pagefind's `.pagefind-ui__message`, debounced 400 ms. Mirrored rather than decorated because Pagefind owns and replaces that subtree. The results drawer is deliberately **not** live — it repaints per keystroke. The region's lifetime is the open dialog, not the page. It is emptied on close, so the write-deduplication guard (an unchanged count is not re-announced) cannot swallow the announcement when a query is repeated after reopening; the observer callback returns early unless the dialog is open, so Pagefind's continued repainting of its own container cannot re-arm the debounce and read a count out over a dismissed dialog; and open restates whatever the panel is showing, since a pointer dismissal leaves the query and its count standing and no mutation is coming for content that never changed. Regression tests: `apps/site/e2e/chrome.spec.ts`, "search result count is announced politely, including no results" (covers the zero case), "typing a query collapses to fewer announcements than keystrokes", "a dismissed dialog announces nothing when Pagefind keeps repainting", "reopening after a pointer dismissal restates the standing count", and "repeating a query after reopening still announces the count". |
