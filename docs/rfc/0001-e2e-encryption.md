...
| **Detection**       | Payload is an E2EE envelope only when the top-level object contains **only** an `e2ee` key (no `basics`, `sections`, or `metadata`). Server rejects mixed plaintext+ciphertext shapes and malformed envelopes (`e2ee.version`, `e2ee.generation`, `e2ee.nonce`, `e2ee.ciphertext` required) with 422. `e2ee.generation` must equal the account's current `e2ee_config.key_generation`, or the next one while a DEK rotation is in progress; any other value is 422, so an envelope sealed under a retired DEK can never be stored. Valid envelopes skip resume-schema checks and the per-field string-length cap (`MAX_STRING_FIELD_LEN`), since the ciphertext is one long base64url string; `MAX_RESUME_JSON_BYTES` and the depth limit still apply. |
...
| Public resume pages               | #65   | Sharing write exists via `update_sharing` (`resume.publish` / `resume.unpublish`); public GET/HTML serving still missing | **Exclude**    | Public pages require server-readable HTML/OG metadata. Block publish when E2EE on.                                                      |
...
4. Re-encrypt each recovery-code backup with a **fresh** `nonce_recovery` so every
   ...
   **must** delete (or mark unusable and refuse to return) every prior `code_hash` +
   ...
   DEK.
5. Increment `e2ee_config.key_generation`. The server accepts envelopes carrying
   ...
   resume or snapshot row still carries the old one (verified in the same
   transaction that increments the counter).
