...
| **AEAD**            | ChaCha20-Poly1305 (RFC 8439). Key = account DEK. Nonce = 12 random bytes per encryption; RFC 8439 requires uniqueness per key, so the rule is stated per key: never repeat a nonce under the same DEK (resume and snapshot envelopes), under the same MK (`wrapped_dek`), or under the same RK (recovery backups, which have the additional freshness rule below). A fresh random nonce per encryption satisfies all three.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
