    ...
    fn convert(&self, data: Self::ValidatedData) -> Result<ResumeData, ParseError> {
        ...
        if let Some(volunteer) = data.volunteer {
            ...
            for v in volunteer {
                ...
                let date = format_date_range(v.start_date.as_deref(), v.end_date.as_deref());
                if !date.is_empty() {
        ...
        if let Some(publications) = data.publications {
            ...
            for p in publications {
                let mut pub_item = Publication::new(p.name.unwrap_or_default());

                if let Some(publisher) = p.publisher {
                    pub_item = pub_item.with_publisher(publisher);
                }

                if let Some(date) = p.release_date {
                    pub_item = pub_item.with_date(date);
                }

                if let Some(summary) = p.summary {
                    pub_item = pub_item.with_summary(summary);
                }
        ...
        if let Some(references) = data.references {
            ...
            for r in references {
                let mut ref_item = Reference::new(r.name.unwrap_or_default());

                // JSON Resume uses "reference" field for the reference text/testimonial
                if let Some(reference_text) = r.reference {
                    ref_item = ref_item.with_summary(reference_text);
                }
