...
export default defineConfig({
  ...
  server: {
    ...
    host: "127.0.0.1",
    fs: {
      // The username rules JSON is shared with the Rust server and lives under
      // crates/, outside the Vite root.
      allow: [
        searchForWorkspaceRoot(process.cwd()),
        fileURLToPath(new URL("../../crates/server/src/auth", import.meta.url)),
      ],
    },
    proxy: {
      "/api": {
